...
from ndb_data.generation.question_to_db import generate_answers
    ...
    try:
        generated_answer = generate_answers(
            ...
            [
                {
                    "qid": qid,
                    "generated": {"question": f[0][0], "derivation": f[0][1]},
...
def generate_joins(hf, facts, qid, question_template, s, r, o):
    yield from generate_joins_filter(
        hf, facts, qid, question_template, s, r, o, is_subject=True
    )
    yield from generate_joins_filter(
        hf, facts, qid, question_template, s, r, o, is_subject=False
    )
    yield from generate_joins_extra(
        hf, facts, qid, question_template, s, r, o, is_subject=True
    )
    yield from generate_joins_extra(
        hf, facts, qid, question_template, s, r, o, is_subject=False
    ...
    with open(args.in_file) as f, open(args.out_file, "w+") as of:
        for line in f:
            ...
            for q in db_questions:
                ...
                facts_grouped, derivations_grouped = group_derivations(
                    ids_grouped, derivations
                )
                question = {
                    "id": qid,
